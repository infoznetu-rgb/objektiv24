...
async function renderHomepage(articles,strategy){
  ...
  const heroImage=hero.image
    ? `<figure class="hero-photo"><img src="${esc(hero.image)}" alt="${esc(hero.imageAlt||hero.title)}" loading="eager" fetchpriority="high" decoding="async"><figcaption>${esc([imageLabel(hero),hero.imageCredit,hero.imageLicense].filter(Boolean).join(" · "))}</figcaption></figure>`
  ...
  const heroBlock=`<section id="suvislosti" class="hero container"><article class="hero-main">${heroImage}<div class="hero-shade"></div><div class="hero-content"><div class="meta-row"><span class="eyebrow">${esc(hero.category)}</span><span>${readingMinutes(hero)} min čítania</span></div><h1>${esc(hero.title)}</h1><p>${esc(hero.summary)}</p><a class="primary-cta" href="${heroUrl}">Čítať ďalej →</a></div></article><aside class="latest"><div class="section-row"><h2>Najnovšie</h2><a href="/clanky/">Všetky →</a></div>${side.map(homepageLatestItem).join("")}<div class="latest-note"><span>Objektív24</span><strong>Správa nestačí. Dávame jej súvislosti.</strong><p>Pri časovo citlivých témach uvádzame dátum overenia a praktický ďalší krok.</p></div></aside></section>`;
