(() => {
  ...
  function visitorId(){
    ...
    if(!value){value=crypto.randomUUID?crypto.randomUUID():Date.now()+'-'+Math.random().toString(36).slice(2);localStorage.setItem(VISITOR_KEY,value)}
    return value;
  }
  ...
  function send(eventType){
    ...
    return fetch(URL+'/rest/v1/site_events',{
      ...
      body:JSON.stringify({path:location.pathname+location.search,article_slug:q.get('slug')||null,referrer:ref,visitor_id:id,event_type:eventType})
    }).catch(()=>{});
  }

  function start(){
    if(started)return;
    started=true;id=visitorId();window.objektiv24Track=send;send('page_view');
  }
  ...
  function choose(value){
    ...
    removeBanner();
    if(value==='yes')start();
    else{localStorage.removeItem(VISITOR_KEY);window.objektiv24Track=noop}
  }
  ...
  function showBanner(force=false){
    ...
    box.id='analytics-consent';box.className='analytics-consent';box.setAttribute('aria-label','Nastavenie analytiky');
    box.innerHTML='<p><strong>Meranie návštevnosti</strong><br>Objektív24 môže po vašom súhlase uložiť náhodný identifikátor do localStorage a použiť ho na základnú štatistiku návštevnosti. <a href="/ako-pracujeme.html#sukromie">Viac o súkromí</a>.</p><div class="analytics-consent-actions"><button class="allow" type="button">Povoliť</button><button class="deny" type="button">Odmietnuť</button></div>';
